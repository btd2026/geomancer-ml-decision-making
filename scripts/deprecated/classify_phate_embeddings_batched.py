...
def process_phate_batch(dataset_dirs, batch_start, batch_size):
    """Process a batch of PHATE embeddings and compute metrics."""
    batch_end = min(batch_start + batch_size, len(dataset_dirs))
    batch_dirs = dataset_dirs[batch_start:batch_end]

    print(f"  Processing batch {batch_start+1}-{batch_end} ({len(batch_dirs)} datasets)...")
...
def main():
    print("="*70)
    print("CLASSIFYING 100 PHATE EMBEDDINGS (BATCHED)")
    print("="*70)
    ...
    # Get list of all dataset directories
    output_dir = Path("/home/btd8/manylatents/outputs/phate_k100_benchmark")
    dataset_dirs = list(output_dir.glob("*"))
    print(f"\nFound {len(dataset_dirs)} PHATE embedding directories")

    # Process in batches
    batch_size = 20  # Process 20 datasets at a time
    all_results = []

    for batch_start in range(0, len(dataset_dirs), batch_size):
        # Process batch
        batch_results = process_phate_batch(dataset_dirs, batch_start, batch_size)
        ...
        if batch_results:
            ...
            X_batch = pd.DataFrame(index=batch_df.index)
            for feat in feature_cols:
                if feat in batch_df.columns:
                    X_batch[feat] = batch_df[feat]
                else:
                    # Fill missing feature with training median
                    X_batch[feat] = X_train[feat].median()

            # Handle NaN/inf in new data
            X_batch = X_batch.replace([np.inf, -np.inf], np.nan).fillna(X_train.median())

            # Predict on batch
            y_pred_encoded = clf.predict(X_batch)
            y_pred_proba = clf.predict_proba(X_batch)
            y_pred = le.inverse_transform(y_pred_encoded)

            # Create batch results
            batch_results_df = batch_df[['dataset_id', 'n_points']].copy()
            batch_results_df['predicted_structure'] = y_pred

            # Add prediction probabilities
            for i, class_name in enumerate(le.classes_):
                batch_results_df[f'prob_{class_name}'] = y_pred_proba[:, i]

            # Add confidence (max probability)
            batch_results_df['confidence'] = y_pred_proba.max(axis=1)

            all_results.append(batch_results_df)
            print(f"    Processed {len(batch_results)} datasets successfully")
    ...
    print(f"\nProcessed {len(results_df)} datasets total")
